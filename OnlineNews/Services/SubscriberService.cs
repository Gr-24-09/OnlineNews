    ...
    public class SubscriptionService : ISubscriptionService
    {
        ...
        public async Task<bool> ChangeSubscriptionTypeAsync(string userId, string subscriptionType)
        {
            var subscriptionTypeObject = await _db.SubscriptionTypes
                .Where(st => st.TypeName.Equals(subscriptionType, StringComparison.OrdinalIgnoreCase))
                .FirstOrDefaultAsync();  
        ...
        {
            return await _db.SubscriptionTypes.Where(st => st.TypeName.ToLower() == subscriptionType
            .ToLower()).Select(st => st.TypeName).FirstOrDefaultAsync();
        }
